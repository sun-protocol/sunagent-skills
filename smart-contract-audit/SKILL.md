...
4.  **Structure** (follow this exact structure):
    ...
    > ⚠️ **MANDATORY MODEL INFO**: The report header **MUST** include a `**Model:**` field identifying the LLM actually executing the audit (e.g. `Claude Opus 4.7`, `GPT-5.4`, `Gemini 3.1 Pro`, `Codex 5.3`, `Grok 4`). Use the human-readable model name as presented to the user in the current session — not an internal slug. If the model version is uncertain, use the best-known family + generation (e.g. `Claude Opus 4.x`). Do **NOT** hallucinate a model name. If you genuinely cannot determine it, write `Unknown (please fill in)` and ask the user.
    >
    > **Filename convention** when multiple audit runs coexist in `.audit/`: save as `.audit/Audit-Report-<model-shorthand>.md` (e.g. `Audit-Report-opus-47.md`, `Audit-Report-gpt-54.md`, `Audit-Report-gemini-31.md`). The canonical `.audit/Audit-Report.md` should still be written as the latest / primary deliverable; the model-tagged filename is an additional copy kept for comparison across models.
    ...
    **Auditor:** AI Security Auditor (DeFi Smart Contract Audit Skill v2.9.0)
    **Model:** [Model Name & Version, e.g. "Claude Opus 4.7"]  <!-- MANDATORY -->
    **Audit Cycle:** [Initial audit | Re-audit N] (if re-audit, link previous report)
...
5.  **Finalize**: Review and clean up the report to ensure:
    *   The header includes **all mandatory fields**: `Date`, `Target Protocol`, `Protocol Type`, `Auditor`, **`Model`** (LLM name + version), and `Audit Cycle`
...
8.  **(Optional) Model-tagged archive**: If `.audit/` already contains model-tagged reports (e.g. `Audit-Report-opus-47.md`), also save a copy as `.audit/Audit-Report-<model-shorthand>.md` so subsequent runs from other models do not overwrite each other's results when compared.
